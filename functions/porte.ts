...
// surface"): Pages Functions are bound to the PROJECT, not to a hostname, so `/api/plan`,
// `/api/plans`, `/api/err` and `/ws` answer on every hostname the project serves. Adding a
// hostname Access does not cover exposes the household's API unless the code decides for itself.
//
...
export type Porte = "foyer" | "inconnue";
...
export function porteDe(request: Request, env: unknown): Porte {
  ...
  return hotes.some((motif) => correspond(hote, motif)) ? "foyer" : "inconnue";
}
